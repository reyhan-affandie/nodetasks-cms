...
import { ModuleForm } from "@/components/forms/transactions.form";
import { format } from "date-fns";
import { useLocale, useTranslations } from "next-intl";
import { ModuleView } from "@/components/views/transactions.view";
...
export default function TransactionsPage() {
  ...
  const columns: TableKeyType[] = [
    ...
    { key: "user.name", label: t("author"), sort: true, isImage: false, isFile: false },
    { key: "client.name", label: t("client"), sort: true, isImage: false, isFile: false },
    { key: "transactionDate", label: t("transaction_date"), sort: true, isImage: false, isFile: false },
    { key: "currency.name", label: t("currency"), sort: true, isImage: false, isFile: false },
    { key: "amount", label: t("amount"), sort: true, isImage: false, isFile: false },
    { key: `stage.name_${locale}`, label: t("stage"), sort: true, isImage: false, isFile: false },
  ];
    ...
    try {
      ...
      if (res?.data?.totalData > 0) {
        const resDataTable = res?.data?.data.map((item: ApiPayload, index: number) => ({
          ...
          ...item,
          transactionDate: item.transactionDate ? format(new Date(item.transactionDate as Date), "MMM dd, yyyy") : "-",
        }));
  ...
  const reload = async () => {
    ...
    if (currentSearch < trap + 500) return true;
  ...
  useEffect(() => {
    if (isLoading === true && data?.length > 0) setIsLoading(false);
  }, [isLoading, data]);
  ...
  useEffect(() => {
    if (selectedData?.data?.formMethod === "view") setAlertViewOpen(true);
    if (selectedData?.data?.formMethod === "update") setSheetOpen(true);
    if (selectedData?.data?.formMethod === "delete") setAlertDeleteOpen(true);
  }, [selectedData]);
  ...
  const tableTitle = t("transactions");
