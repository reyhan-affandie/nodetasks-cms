...
import { getSubList, getOne } from "@/actions/actions";
...
import { ModuleForm } from "@/components/forms/privileges.form";
import { format } from "date-fns";
import { useTranslations } from "next-intl";
import { ModuleView } from "@/components/views/privileges.view";
import { useParams } from "next/navigation";

const api = "privileges";
...
export default function PrivilegesPage() {
  ...
  const [parentData, setParentData] = useState<ApiPayload>();
  ...
  const columns: TableKeyType[] = [
    ...
    { key: "role.name", label: t("role"), sort: true, isImage: false, isFile: false },
    { key: "feature.name", label: t("feature"), sort: true, isImage: false, isFile: false },
    ...
    { key: "privilegeCreate", label: t("create"), sort: false, isImage: false, isFile: false },
    { key: "privilegeRead", label: t("read"), sort: false, isImage: false, isFile: false },
    { key: "privilegeUpdate", label: t("update"), sort: false, isImage: false, isFile: false },
    { key: "privilegeDelete", label: t("delete"), sort: false, isImage: false, isFile: false },
  ];

  const fetchData = useCallback(async () => {
    try {
      setIsLoading(true);
      const getParent = await getOne(parents, parentId);
      setParentData(getParent?.data);
      const res = await getSubList(api, parent, parentId, page, limit, search, sort, order);
  ...
  }, [parentId, page, limit, search, sort, order]);
