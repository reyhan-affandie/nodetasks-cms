...
import { prioritiesValidation, prioritiesAction } from "@/actions/priorities.actions";
...
export function ModuleForm({
  ...
  const [formState, formAction] = useActionState(prioritiesAction, selectedData);
  ...
  const handleSubmit = async () => {
    ...
    const result = await prioritiesValidation(formData);
    if (result) {
      setErrors(result);
